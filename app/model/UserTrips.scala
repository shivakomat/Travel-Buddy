...
case class UserTrips(userId: Int, trips: Seq[Trip],
                     ...
                     stayItems: Seq[StayItem],
                     foodPlaceItems: Seq[FoodPlaceItem],
                     placesToVisit: Seq[PlacesToVisit])
...
//    .add(TripPlace(id = 2, name = "Dubrovnik", tripId = 1))
  ...
  def byUser(userId: Int): UserTrips = {
    ...
    val flightItems = BucketItems.flightItems(userId)
    val stayItems = BucketItems.stayItems(userId)
    val placesToVisit = BucketItems.placesToVisit(userId)
    val foodPlaces = BucketItems.foodPlaces(userId)
    UserTrips(userId, trips, tripPlaces, flightItems, stayItems, foodPlaces, placesToVisit)
  }
